...
def get_non_standard_section_ratio(row, good_sections=None):
    ...
    list_of_names = [
        k.replace("section_sections_", "").replace("_entropy", "") for k in labels
    ]
...
def transform_section(df_flat):
    ...
    df_section = df_flat[
        df_flat.columns[df_flat.columns.str.startswith("section_")]
    ].copy()

    # drop columns with size and vsize
    df_section.drop(
        df_section.columns[df_section.columns.str.contains("_size")],
        axis=1,
        inplace=True,
    )
    df_section.drop(
        df_section.columns[df_section.columns.str.contains("_vsize")],
        axis=1,
        inplace=True,
    )
    ...
    df_section["section_has_non_standard_entry"] = np.where(
        df_section["section_entry"].isin(standard_entries), 0, 1
    )

    # Section - sections

    # count number of sections
    df_section["section_cnt"] = df_section[
        df_section.columns[df_section.columns.str.contains("_entropy")]
    ].apply(lambda x: sum(x >= 0), axis=1)

    # count sections which contain high entropy (>6)
    df_section["section_high_entropy_cnt"] = df_section[
        df_section.columns[df_section.columns.str.contains("_entropy")]
    ].apply(lambda x: sum(x >= 6), axis=1)

    # create new column: section_high_entropy ratio
    df_section["section_high_entropy_ratio"] = (
        df_section["section_high_entropy_cnt"] / df_section["section_cnt"]
    )

    # create new column: _ section entropy
    df_section["section_entropy_max"] = df_section[
        df_section.columns[df_section.columns.str.contains("_entropy")]
    ].max(axis=1)
    ...
    df_section["section_has_non_standard_sections_ratio"] = df_section[
        df_section.columns[df_section.columns.str.endswith("_entropy")]
    ].apply(lambda x: get_non_standard_section_ratio(x, standard_entries), axis=1)

    # drop of non relevant columns
    df_section.drop(
        df_section.columns[df_section.columns.str.contains("section_sections")],
        axis=1,
        inplace=True,
    )
    df_section.drop(
        df_section.columns[df_section.columns.str.contains("section_entry")],
        axis=1,
        inplace=True,
    )
...
def transform_data_directories(df_flat):
    ...
    df_data_directories = df_flat[
        df_flat.columns[df_flat.columns.str.startswith("datadirectories")]
    ].copy()

    # removing columns describing virtual size
    df_data_directories.drop(
        df_data_directories.columns[df_data_directories.columns.str.contains("_vsize")],
        axis=1,
        inplace=True,
    )

    # removing columns with correlation with label close to 0
    data_directories_to_drop = [
        ...
        "datadirectories_BOUND_IMPORT_size",
        "datadirectories_IAT_size",
        "datadirectories_CLR_RUNTIME_HEADER_size",
    ]
...
def transform_byte_entropy(df_flat):
    ...
    df_data_directories = df_flat[
        df_flat.columns[df_flat.columns.str.startswith("byteentropy")]
    ].copy()

    # calculating average of most important columns
    average_of_bytes_240_255 = (
        df_data_directories.loc[:, "byteentropy_240":"byteentropy_255"].sum(axis=1)
    ) / 16

    return pd.DataFrame(average_of_bytes_240_255, columns=["average_of_bytes_240_255"])
...
def transform_histogram(df_flat):
    ...
    temporal = df_flat[
        df_flat.columns[df_flat.columns.str.startswith("histogram")]
    ].copy()

    # calculating max value from histograms
    max_hist = temporal.loc[:, "histogram_0":"histogram_255"].max(axis=1)

    return pd.DataFrame(max_hist, columns=["max_hist"])
...
def transform_imports(df_flat):
    ...
    inputs:
        df_flat: flatten dataframe
    output:
        contains_malicious_imports: column with 1 if contains functions, 0
    """
    malicious_functions = [
        "imports.kernel32.dll-MapViewOfFile",
        "imports.kernel32.dll-IsDebuggerPresent",
        "imports.kernel32.dll-GetThreadContext",
        "imports.kernel32.dll-ReadProcessMemory",
        "imports.kernel32.dll-ResumeThread",
        "imports.kernel32.dll-WriteProcessMemory",
        "imports.kernel32.dll-SetFileTime",
        "imports.kernel32.dll-MapViewOfFile",
        "imports.kernel32.dll-ResumeThread",
        "imports.user32.dll-SetWindowsHookExW",
        "imports.kernel32.dll-CreateToolhelp32Snapshot",
        "imports.user32.dll-SetWindowsHookExW",
        "imports.advapi32.dll-CryptGenRandom",
        "imports.advapi32.dll-OpenThreadToken",
        "imports.advapi32.dll-CryptAcquireContextW",
        "imports.advapi32.dll-DuplicateTokenEx",
    ...
    return pd.DataFrame(df_contains_malicious_functions, columns=["has_suspicious_dll"])
